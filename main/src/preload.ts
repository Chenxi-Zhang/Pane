...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  events: {
    ...
    onPanelActivityStatus: (callback: (data: { panelId: string; sessionId: string; status: 'active' | 'idle' | 'unviewed'; lastActivityAt?: string }) => void) => {
      const wrappedCallback = (_event: Electron.IpcRendererEvent, data: { panelId: string; sessionId: string; status: 'active' | 'idle' | 'unviewed'; lastActivityAt?: string }) => callback(data);
